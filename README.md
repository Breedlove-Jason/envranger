...
**Requires Node.js 22+ and npm.** Version 0.2.0 is available from this GitHub repository. It is **not published to the npm registry**; do not install a similarly named registry package expecting this project.
...
- **Local files, no cloud account.** No telemetry, uploads, encryption, vault service or provider synchronization. Use your hosting provider or secret manager for production secrets.
...
- **Previews first.** Editing commands require `--write`. Existing changed files get a private backup in `.envranger/backups/`. Backups contain secrets too; keep them ignored and remove them when no longer needed.
