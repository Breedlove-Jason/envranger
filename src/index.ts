...
export function loadConfig(root = process.cwd()): Config {
  ...
  const defaults: Config = {scanDirs:['.'], ignore:[], required:[], optional:[], deprecated:[], envFile:'.env', profiles:{}, routes:{}};
...
export function loadFiles(root: string, files: string[]) {
  ...
  for (const file of files) {
    ...
    for (const [key,value] of Object.entries(values(doc))) { env[key]=value; sources[key]=file; }
  }
  return {env,sources};
}
...
export function routePlan(root: string, cfg: Config, env: Record<string,string>, overwrite = false) {
  ...
  for (const [file, keys] of Object.entries(cfg.routes)) {
    ...
    targets.add(target);
    let doc = readDocument(target,true); const existing = values(doc);
    ...
    for (const key of changed) { content = update(doc,key,env[key]); doc = parse(content); }
